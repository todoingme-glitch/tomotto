...
(function initBottomTab() {
  ...
  switchTab = function(tabId, opts = {}) {
    ...
    // 파노라마 배경 포지션 전환
    const _bgPos = { personal: '0% 30%', social: '50% 30%', log: '100% 30%' };
    document.querySelector('.container')?.style.setProperty('background-position', _bgPos[tabId] || '0% 30%');
    // 개인탭으로 전환 시 타이머가 실행 중이면 타이머 다이얼로그 자동 열기
    if (tabId === 'personal' && typeof timer !== 'undefined' && timer.isRunning) {
      const td = document.getElementById('dialog-timer');
      if (td && !td.open) td.showModal();
    }

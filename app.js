...
(function initBottomTab() {
  ...
  switchTab = function(tabId, opts = {}) {
    ...
    document.querySelector('.container')?.setAttribute('data-tab', tabId);
    if (tabId === 'personal') {
      setTimeout(() => {
        try {
          if (timer.isRunning) {
            document.getElementById('card-timer')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
        } catch (_) {}
      }, 0);
    }
...
(function initPublicLobbyEvents() {
  ...
  document.getElementById('pubLobbyReadyBtn')?.addEventListener('click', () => {
    ...
    if (btn?.dataset.siegeGacha === 'true') {
      ...
      switchTab('personal');
      setTimeout(() => {
        const targetCard = categories.length === 0 ? 'card-todo' : 'card-gacha';
        document.getElementById(targetCard)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 200);
    } else {
      togglePublicReady();
    }

...
// =====================================================
/** 로비가 닫혀있을 때 인사 수신 → 화면 상단→중단 이모지 샤워 + 토스트 */
function showEmojiRainGreeting(fromNick, emojis = ['🔥', '✨', '⚡', '💥']) {
...
async function enrichLobbyWithPartnerBadges(players) {
  ...
  try {
    ...
    // 파트너 발견 시 로비 인사 메시지 + 인사 버튼 (기존 것 교체)
    const $modal  = document.getElementById('publicLobbyModal');
    ...
    if (knownInLobby.length > 0) {
      ...
      $greetBtn.textContent = '버튼을 눌러 인사를 보내세요! 🔥';
      const _topNick = top.nick;
      let _rainBusy = false;
      $greetBtn.addEventListener('click', () => {
        if (_rainBusy) return;
        const $m = document.getElementById('publicLobbyModal');
        if (!$m?.open) return;
        // 클릭 시점에 DOM 재조회 (재렌더링으로 stale 방지)
        const $list = document.getElementById('pubLobbyPlayers');
        const $fresh = [...($list?.querySelectorAll('.pub-lobby-player:not(.is-me)') || [])]
          .find(el => el.querySelector('.pub-lobby-nick')?.firstChild?.textContent?.trim() === _topNick);
        if (!$fresh) return;
        _rainBusy = true;
        showEmojiRainOnElement($fresh, ['🔥', '✨', '⚡', '💥']);
        // 상대방에게 broadcast
      ...
      $status?.insertAdjacentElement('beforebegin', $msg);
      $status?.insertAdjacentElement('beforebegin', $greetBtn);
    }
  } catch {}
}
  ...
  publicLobbyChannel = sb.channel(`pub-lobby-${battleId}`)
    ...
    .on('broadcast', { event: 'emoji_rain' }, ({ payload }) => {
      ...
      if ($modal?.open) {
        ...
        // 로비 닫혀있음 → 화면 상단→중단 이모지 샤워 + 토스트 알림
        showEmojiRainGreeting(payload.from);
      }
    })
    .subscribe();
}
